...
export default function EasyLevel1() {
  ...
  const endGame = (inputValue = userInput) => {
    ...
    if (!hasGoodSpeed) {
      detailedFeedback.push(`❌ Speed: ${wpm} WPM (need 20+ WPM)`);
    } else {
    ...
    if (!hasLowMistakes) {
      detailedFeedback.push(`❌ Mistakes: ${wordMistakes} (max 2 allowed)`);
    } else {
  ...
  // ignore errors for now
    ...
    setResults({
      ...
      requirements: {
        ...
        mistakes: { required: 2, achieved: wordMistakes, passed: hasLowMistakes },
