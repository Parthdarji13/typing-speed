...
export default function EasyLevel3() {
  ...
  const paragraph = "The quick brown fox jumps over the lazy dog. This is a longer sentence to practice typing skills. Learning to type faster takes time and practice. You need to focus on accuracy and speed.";
  ...
  const endGame = (inputValue = userInput) => {
    ...
    const elapsedSec = isFinishedTyping
      ...
      : Math.max(1, startTime ? (45 - timeLeft) : 1);
    ...
    const hasGoodSpeed = wpm >= 30; // Increased minimum speed for level 3
    const hasGoodAccuracy = parseFloat(accuracy) >= 85;
    const hasLowMistakes = wordMistakes <= 4; // Allow more mistakes for longer text
    ...
    if (!hasGoodSpeed) {
      detailedFeedback.push(`❌ Speed: ${wpm} WPM (need 30+ WPM)`);
    } else {
      detailedFeedback.push(`✅ Speed: ${wpm} WPM`);
    }
    
    if (!hasGoodAccuracy) {
      detailedFeedback.push(`❌ Accuracy: ${accuracy}% (need 85%+)`);
    } else {
      detailedFeedback.push(`✅ Accuracy: ${accuracy}%`);
    }
    
    if (!hasLowMistakes) {
      detailedFeedback.push(`❌ Mistakes: ${wordMistakes} (max 4 allowed)`);
    } else {
    ...
    if (isCompleted) {
        message = "🎉 Level Completed! All requirements met!";
                 ProgressTracker.completeLevel('easy', 3, {
           wpm,
    ...
    setResults({
      ...
      isCompleted,
      requirements: {
        speed: { required: 30, achieved: wpm, passed: hasGoodSpeed },
        accuracy: { required: 85, achieved: parseFloat(accuracy), passed: hasGoodAccuracy },
        mistakes: { required: 4, achieved: wordMistakes, passed: hasLowMistakes },
        ...
        <div className="w-full max-w-2xl bg-white text-black p-6 rounded-2xl shadow-lg">
          ...
          {!started && !finished && (
            <div className="text-center">
              <button
                onClick={handleStart}
                className="bg-yellow-400 text-black px-6 py-3 rounded-lg shadow-lg text-xl font-bold hover:bg-yellow-500 transition"
              >
                Start
              </button>
            </div>
          ...
          {started && !finished && (
            <>
              ...
              <textarea
                ...
                disabled={finished}
                className="w-full h-32 border-2 border-gray-300 rounded-lg p-3 text-lg focus:outline-none focus:ring-2 focus:ring-blue-400"
              />
            </>
          )}
            ...
            <div className="text-center">
              ...
              <div className="mt-6 flex justify-center gap-4">
                {results.isCompleted ? (
                  ...
                  <button
                    onClick={restartGame}
                    className="bg-yellow-400 text-black px-5 py-2 rounded-lg hover:bg-yellow-500 transition"
                  >
                    Try Again
                  </button>
                )}
