...
export default function HardLevel2() {
  ...
  const endGame = (inputValue = userInput) => {
    ...
    const elapsedSec = isFinishedTyping
      ...
      : Math.max(1, startTime ? (180 - timeLeft) : 1);
    ...
    const hasGoodSpeed = wpm >= 60; // Higher speed requirement for hard level 2
    const hasGoodAccuracy = parseFloat(accuracy) >= 92;
    const hasLowMistakes = wordMistakes <= 12; // Allow more mistakes for longer text
    ...
    if (!hasGoodSpeed) {
      detailedFeedback.push(`❌ Speed: ${wpm} WPM (need 60+ WPM)`);
    } else {
    ...
    if (!hasLowMistakes) {
      detailedFeedback.push(`❌ Mistakes: ${wordMistakes} (max 12 allowed)`);
    } else {
    ...
    if (isCompleted) {
        message = "🎉 Level Completed! All requirements met!";
                 ProgressTracker.completeLevel('hard', 2, {
           wpm,
    ...
    setResults({
      ...
      isCompleted,
      requirements: {
        speed: { required: 60, achieved: wpm, passed: hasGoodSpeed },
        accuracy: { required: 92, achieved: parseFloat(accuracy), passed: hasGoodAccuracy },
        mistakes: { required: 12, achieved: wordMistakes, passed: hasLowMistakes },
          ...
          {finished && results && (
            <div className="text-center">
              ...
              <div className="mt-6 flex justify-center gap-4">
                {results.isCompleted ? (
                  <button
                    onClick={() => navigate("/levels/hardlevel3")}
