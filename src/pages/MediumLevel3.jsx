...
export default function MediumLevel3() {
  ...
  const endGame = (inputValue = userInput) => {
    ...
    const elapsedSec = isFinishedTyping
      ...
      : Math.max(1, startTime ? (120 - timeLeft) : 1);
    ...
    const hasGoodSpeed = wpm >= 45; // Higher speed requirement for medium level 3
    const hasGoodAccuracy = parseFloat(accuracy) >= 90; // Higher accuracy requirement
    const hasLowMistakes = wordMistakes <= 8; // Allow more mistakes for longer text
    ...
    if (!hasGoodSpeed) {
      detailedFeedback.push(`❌ Speed: ${wpm} WPM (need 45+ WPM)`);
    } else {
      detailedFeedback.push(`✅ Speed: ${wpm} WPM`);
    }
    
    if (!hasGoodAccuracy) {
      detailedFeedback.push(`❌ Accuracy: ${accuracy}% (need 90%+)`);
    } else {
      detailedFeedback.push(`✅ Accuracy: ${accuracy}%`);
    }
    
    if (!hasLowMistakes) {
      detailedFeedback.push(`❌ Mistakes: ${wordMistakes} (max 8 allowed)`);
    } else {
    ...
    if (isCompleted) {
        message = "🎉 Level Completed! All requirements met!";
                 ProgressTracker.completeLevel('medium', 3, {
           wpm,
    ...
    setResults({
      ...
      isCompleted,
      requirements: {
        speed: { required: 45, achieved: wpm, passed: hasGoodSpeed },
        accuracy: { required: 90, achieved: parseFloat(accuracy), passed: hasGoodAccuracy },
        mistakes: { required: 8, achieved: wordMistakes, passed: hasLowMistakes },
