...
export default function EasyLevel2() {
  ...
  const paragraph = "The quick brown fox jumps over the lazy dog. This is a longer sentence to practice typing skills.";
  ...
  const endGame = (inputValue = userInput) => {
    ...
    const elapsedSec = isFinishedTyping
      ...
      : Math.max(1, startTime ? (30 - timeLeft) : 1);
    ...
    const hasGoodSpeed = wpm >= 25; // Increased minimum speed for level 2
    const hasGoodAccuracy = parseFloat(accuracy) >= 85;
    const hasLowMistakes = wordMistakes <= 3; // Allow slightly more mistakes for longer text
    ...
    if (!hasGoodSpeed) {
      detailedFeedback.push(`❌ Speed: ${wpm} WPM (need 25+ WPM)`);
    } else {
    ...
    if (!hasLowMistakes) {
      detailedFeedback.push(`❌ Mistakes: ${wordMistakes} (max 3 allowed)`);
    } else {
    ...
    if (isCompleted) {
        message = "🎉 Level Completed! All requirements met!";
                 ProgressTracker.completeLevel('easy', 2, {
           wpm,
    ...
    setResults({
      ...
      isCompleted,
      requirements: {
        speed: { required: 25, achieved: wpm, passed: hasGoodSpeed },
        accuracy: { required: 85, achieved: parseFloat(accuracy), passed: hasGoodAccuracy },
        mistakes: { required: 3, achieved: wordMistakes, passed: hasLowMistakes },
