...
export default function MediumLevel2() {
  ...
  const endGame = (inputValue = userInput) => {
    ...
    const elapsedSec = isFinishedTyping
      ...
      : Math.max(1, startTime ? (90 - timeLeft) : 1);
    ...
    const hasGoodSpeed = wpm >= 40; // Higher speed requirement for medium level 2
    const hasGoodAccuracy = parseFloat(accuracy) >= 88;
    const hasLowMistakes = wordMistakes <= 6; // Allow more mistakes for longer text
    ...
    if (!hasGoodSpeed) {
      detailedFeedback.push(`❌ Speed: ${wpm} WPM (need 40+ WPM)`);
    } else {
    ...
    if (!hasLowMistakes) {
      detailedFeedback.push(`❌ Mistakes: ${wordMistakes} (max 6 allowed)`);
    } else {
    ...
    if (isCompleted) {
        message = "🎉 Level Completed! All requirements met!";
                 ProgressTracker.completeLevel('medium', 2, {
           wpm,
    ...
    setResults({
      ...
      isCompleted,
      requirements: {
        speed: { required: 40, achieved: wpm, passed: hasGoodSpeed },
        accuracy: { required: 88, achieved: parseFloat(accuracy), passed: hasGoodAccuracy },
        mistakes: { required: 6, achieved: wordMistakes, passed: hasLowMistakes },
